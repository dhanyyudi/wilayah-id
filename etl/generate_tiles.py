...
Workflow:
  ...
  3. Merge all layers into one .mbtiles
  4. Extract to static .pbf directory via mb-util
...
Prerequisites:
    ...
    - PostGIS data loaded (run import_all.py first)
"""
...
TILE_WORK_DIR = PROJECT_ROOT / "tiles-work"  # temp working dir
TILE_OUTPUT_DIR = PROJECT_ROOT / "public" / "tiles"  # final output

# Layer definitions with zoom ranges per PRD
LAYERS = [
    {
        "name": "provinsi",
        "min_zoom": 3,
    ...
    },
    {
        ...
        """,
        "properties": ["kode_kab", "kode_prov", "nama_kabupaten", "tipe", "nama_provinsi", "area_km2"],
    },
    {
        "name": "kecamatan",
        "min_zoom": 10,
...
def rename_tiles_to_pbf(layer_dir):
    ...
    # mb-util outputs: {z}/{x}/{y} (no extension) in TMS scheme
    # We need: {z}/{x}/{y}.pbf
    renamed = 0
...
def main():
    print("╔══════════════════════════════════════════════╗")
    print("║  wilayah-id — Vector Tile Generation          ║")
    print("╚══════════════════════════════════════════════╝")
    ...
    # Cleanup working dir (keep GeoJSON for reference)
    print(f"\n  Working files in: {TILE_WORK_DIR}")
    print(f"  (delete manually when no longer needed)")

    print("\n🎉 Tile generation complete!")
    print("   Next: pnpm dev and configure MapLibre sources")
